...
| server.tls.certSecretName | string | `"openshell-server-tls"` | K8s secret (type kubernetes.io/tls) with tls.crt and tls.key for the gateway server listener. The secret may be created by the built-in pkiInitJob, by cert-manager, or from externally managed PEM/X.509 certificate material. |
