...
OpenShell uses two distinct certificate paths in gateway configuration:

- Gateway listener TLS
  - configured under `[openshell.gateway.tls]`
  - supplies the server certificate and optional client CA trust for the
    gateway listener itself
- Sandbox transport client TLS
  - configured in driver-specific settings such as `guest_tls_ca`,
    `guest_tls_cert`, `guest_tls_key`, or `client_tls_secret_name`
  - supplies the client certificate bundle supervisors use when they call back
    into the gateway over HTTPS

The certificate material for either path may come from:

- OpenShell-generated bootstrap PKI
- operator-provided files or Kubernetes Secrets
- cert-manager-managed Secrets
- externally managed PEM/X.509 certificate material

This page documents how the gateway consumes those inputs. It does not require
OpenShell to own certificate issuance.
...
# Gateway listener TLS (distinct from the per-driver guest_tls_* sandbox
...
When the gateway listener uses externally managed TLS material, Docker cannot
auto-detect a working HTTPS callback path on its own. In that mode, configure
either:

- an explicit `grpc_endpoint`, or
- the full `guest_tls_ca`, `guest_tls_cert`, and `guest_tls_key` bundle so
  Docker can auto-detect `https://host.openshell.internal:<gateway-port>`

The self-generated local PKI path already provides this guest TLS bundle
automatically.
...
When the gateway listener uses externally managed TLS material, Podman also
requires either:

- an explicit `grpc_endpoint`, or
- the full `guest_tls_ca`, `guest_tls_cert`, and `guest_tls_key` bundle so the
  driver can auto-detect `https://host.containers.internal:<gateway-port>`

The self-generated local PKI path already wires these guest TLS files by
default.
