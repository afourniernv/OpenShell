...
# Private IDIRA provider-refresh PoC. The API key is mounted separately.
...
The private IDIRA PoC adds one narrowly scoped external provider-refresh
backend. It reads an existing IDIRA or Conjur-compatible variable and sends the
value through OpenShell's normal provider credential storage and L7 injection
path.
...
`base_url`, `account`, `login`, `reference_prefix`, and `api_key_path` are
required. The gateway reads the API key from `api_key_path` when it
authenticates; inline API-key fields are rejected. `ca_cert_path` optionally
adds a PEM trust root.
`timeout_seconds` defaults to `30`, and `auth_token_ttl_seconds` defaults to
`300`. Authentication tokens remain in gateway memory.

`reference_prefix` is operator-owned. The gateway resolves provider references
under `<reference_prefix>/<workspace-id>/<reference>`. It pins the immutable
workspace UUID when refresh is configured, so reusing a deleted workspace name
does not expose the old workspace's IDIRA variables. A workspace administrator
cannot use the shared gateway credential to read another workspace's variables.
Provider references must be relative paths without `.` or `..` components.

This is eager retrieval, not JIT. The gateway fetches on the provider refresh
cadence and stores the result at rest in the configured OpenShell credential
store. Sandboxes do not contact IDIRA and do not receive the gateway's IDIRA
credential. The PoC does not use the supervisor middleware API and does not
implement IDIRA-backed `CredentialDriver` create, update, or delete operations.
Because IDIRA variables have no expiry in this path, a refresh failure leaves
the last successfully fetched value active. This PoC has no maximum-staleness
