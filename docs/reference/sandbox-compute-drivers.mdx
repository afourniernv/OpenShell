...
| `client_tls_secret_name` | `server.tls.clientTlsSecretName` | Mount the sandbox transport client TLS bundle from a Kubernetes Secret. This bundle is used by supervisors when they call back into the gateway over HTTPS. |
