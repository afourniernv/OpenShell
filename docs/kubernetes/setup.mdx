...
| cert-manager | No | Refer to [Managing Certificates](/kubernetes/managing-certificates). Use cert-manager when you want controller-managed certificate issuance and renewal. |
| Externally managed certificate Secrets | No | Refer to [Managing Certificates](/kubernetes/managing-certificates). Use this mode when an enterprise CA workflow or adjacent controller creates the Secrets and OpenShell only consumes them. |
