...
The gateway always expects transport certificate material for HTTPS and sandbox
supervisor callbacks, but the Helm chart does not have to generate that
material itself. Deployments can use the built-in `pkiInitJob`,
`cert-manager`, or externally managed certificate Secrets. The client bundle in
the `openshell-client-tls` secret is used internally by sandbox supervisors,
not for granting access to individual users.
