...
| `external` | An external process normally updates current credentials through `openshell provider update`. The private IDIRA PoC also recognizes `backend=idira` and lets the gateway fetch an existing variable. |
...
`openshell provider refresh configure` accepts the gateway-mintable strategies
`oauth2-refresh-token`, `oauth2-client-credentials`,
`google-service-account-jwt`, and `aws-sts-assume-role`. The private IDIRA PoC
also accepts `external` only with `backend=idira`, an existing variable
`reference`, and an optional `refresh_interval_seconds`. Other `external`
patterns still use `openshell provider update`.
...
| `external` with `backend=idira` (private PoC) | `backend=idira`, `reference`, and optional `refresh_interval_seconds` from `60` through `86400` (default `300`). |
...
Configure eager IDIRA retrieval in the private PoC build after adding
`[openshell.gateway.idira]` to the gateway configuration:

```shell
openshell provider refresh configure github-idira \
  --credential-key GITHUB_TOKEN \
  --strategy external \
  --material backend=idira \
  --material reference=providers/github/api-key \
  --material refresh_interval_seconds=300

openshell provider refresh rotate github-idira \
  --credential-key GITHUB_TOKEN
```

The IDIRA reference is non-secret metadata and is relative to the provider's
workspace. The gateway reads
`<reference_prefix>/<workspace-id>/<reference>`, where the gateway operator owns
`reference_prefix` and OpenShell pins the workspace's immutable UUID. Do not
pass the gateway IDIRA API key as refresh material;
mount it at the configured `api_key_path`. The fetched value is stored at rest
through OpenShell's selected credential store, while the provider record
retains only an opaque handle. Sandboxes never contact IDIRA. This path is
eager, not JIT, and does not use supervisor middleware or an IDIRA
`CredentialDriver`. An IDIRA refresh failure leaves the last successfully
fetched value active because this PoC has no maximum-staleness or fail-closed
guarantee.
